...
test('quitPromptFor names the running chats', () => {
  ...
  assert.ok(prompt)
  assert.equal(prompt.message, `${MAIN_BRAND.productName} is still working on 2 chats.`)
...
test('quitPromptFor summarizes past the list cap and counts untitled work', () => {
  ...
  assert.ok(prompt)
  assert.equal(prompt.message, `${MAIN_BRAND.productName} is still working on 9 chats.`)
...
test('quitPromptFor speaks singular for one chat', () => {
  ...
  assert.ok(prompt)
  assert.equal(prompt.message, `${MAIN_BRAND.productName} is still working on 1 chat.`)
