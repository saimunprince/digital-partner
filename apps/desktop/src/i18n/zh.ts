...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 {brand} 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 {brand} 桌面版…'
    },
    errors: {
      backgroundExited: '{brand} 后台进程已退出。',
      backgroundExitedDuringStartup: '{brand} 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 {brand} 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    installMethodUnsupportedTitle: '不受支持的安装方式',
    updateHermes: '更新 {brand}',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 {brand}。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: '{brand} 正在等待你的回应。',
      turnDoneTitle: '{brand} 已完成',
      turnDoneBody: '',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 {brand} 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 {brand} 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '运行在 {brand} 后端——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        empty: '尚未安装智能体插件。',
        ...
        updateBackendToManage: '请更新 {brand} 后端以便在桌面端管理此插件。',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 {brand} 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: '{brand} 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: '{brand} 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '后台回合错误。'
        ...
        plugin: {
          label: '插件通知',
          description: '{brand} 在后台时，桌面插件发送了通知。'
        }
      },
      test: '发送测试通知',
      testTitle: '{brand}',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 {brand} 跟随系统设置。',
      ...
      reactionsTitle: '消息回应',
      reactionsDesc: 'iMessage 风格的表情回应 — 你可以给消息添加回应，{brand} 也能回应你的消息。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 {brand} 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 {brand}，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 {brand} 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: '{brand} 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: '{brand} 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: '{brand} 如何处理需要显式审批的命令。',
    ...
    about: {
      ...
      automaticUpdatesDesc: '{brand} 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    quickEntry: {
      enabledTitle: '快速输入',
      enabledDesc: '用全局快捷键在任何地方唤出一个小输入框，无需打开 {brand} 即可发送提示。',
    ...
    connections: {
      ...
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 {brand} 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 {brand} 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Hermes Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 {brand} 安装。',
      labelTitle: '名称',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        '{brand} 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 {brand} 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      modeTitle: '连接模式',
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 {brand} 后端。这是默认方式，并且可离线工作。',
      inheritTitle: '使用默认网关',
      inheritDesc: '移除此 profile 的自定义覆盖并使用默认连接。',
      remoteTitle: '远程网关',
      remoteDesc: '将此桌面外壳连接到远程 {brand} 后端。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: '{brand} 将使用已保存设置重新连接（界面保持打开）。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · {brand} ${version}` : ''}`,
      ...
      sshTitle: '通过 SSH 连接',
      sshDesc:
        '{brand} 会通过 SSH 在远程启动并以隧道连接到本应用——无需自行启动或暴露任何服务。前提：已具备到该主机的密钥 SSH 访问。',
      ...
      sshReachable: (host, platform) => `可连接：${host}（${platform}）——已找到 {brand}`,
      sshIncompleteHost: '连接前请输入 SSH 主机。',
      sshErrUnreachable: '无法通过 SSH 连接到该主机。请检查主机、端口和网络。',
      sshErrAuth:
        'SSH 认证失败。请将密钥加载到 ssh-agent（ssh-add），或在 ~/.ssh/config 中设置 IdentityFile——{brand} 以非交互方式运行 ssh。',
      ...
      sshErrPlatform: '不支持的远程平台。{brand} 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
      sshErrTimeout: 'SSH 连接超时。主机可能无法访问或处于休眠状态。',
      sshErrUpdateRequired: '使用 Desktop SSH 连接前，请更新远程主机上的 {brand}。',
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。{brand} 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
      ...
      localEndpoint: {
        title: '本地 / 自定义端点',
        description: '将 {brand} 指向任意 OpenAI 兼容端点（Zyphra、vLLM、llama.cpp、Ollama 等）。'
      },
      loading: '正在加载提供方...'
    },
    sessions: {
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 {brand} 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 {brand} 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 {brand} 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 {brand} 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 {brand} 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `{brand} ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 {brand} 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。{brand} 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 {brand} 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 {brand} 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 {brand}…',
    placeholderReconnecting: '正在重新连接 {brand}…',
    ...
    attachUrlDesc: '{brand} 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 {brand} 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 {brand} 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 {brand} 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 {brand}，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 {brand} 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 {brand} 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      '{brand} 更新器会在自己的窗口中接管，并在完成后自动重新打开 {brand}。更新期间请不要自行重新打开 {brand}。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 {brand} 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 {brand} 会自动重新打开。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    setupChoiceTitle: '设置 {brand}',
    setupChoiceDesc: '将此应用连接到你已运行的 {brand} 网关，或在这台电脑上本地安装 {brand}。',
    connectExistingTitle: '连接到现有 {brand}',
    connectExistingShort: '连接现有环境',
    connectExistingDesc: '使用会话令牌或浏览器登录连接远程后端。不会启动本地安装。',
    installLocalTitle: '本地安装 {brand}',
    installLocalDesc: '下载 {brand}，创建 Python 环境，并在这台电脑上运行后端。',
    localStartUnavailable: '无法启动本地安装。请重启 {brand} 后重试。',
    remoteSetupTitle: '连接到现有 {brand}',
    remoteSetupDesc: '输入网关 URL。{brand} 会检测需要令牌还是浏览器登录。',
    remoteUrlTitle: '网关 URL',
    remoteUrlDesc: '使用 {brand} 网关的基础 URL；远程地址请包含 https://。',
    remoteUrlPlaceholder: 'https://gateway.example.com/hermes',
    probing: '正在检测网关认证方式...',
    probeError: '无法连接到该 {brand} 网关。',
    identityProvider: '你的身份提供方',
    authTitle: '认证',
    ...
    backToSetup: '返回',
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 {brand} Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 {brand} CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 {brand} 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。{brand} 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 {brand} Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: '{brand} 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 {brand}…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 {brand} 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 {brand} 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — {brand} 会自动连接',
    ...
    authorizeThere: '请在那里授权 {brand}。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 {brand}，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。{brand} 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: '{brand} 正在重启...',
      askRestart: '让 {brand} 重启服务器',
      lookingRestart: taskId => `{brand} 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: '{brand} 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `{brand} 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: '{brand} 无法重启服务器。',
      stillWorking: '{brand} 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: '{brand} 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。{brand} 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: '{brand} 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: '{brand} 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 {brand} 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 {brand} 桌面版以保存图片',
