...
# The desktop spawns `hermes_cli.main serve` from here on launch. A separate
# hermes-gateway.service may also be running from the INSTALLED copy under
# ~/.hermes — that one carries the messaging integrations and is not what
# serves this app, so leave it alone. To confirm which engine is answering:
#
#   pgrep -af "hermes_cli.main"   # the desktop's is a child of electron
#
# The venv beside this script is the one ensure_python prepared.
